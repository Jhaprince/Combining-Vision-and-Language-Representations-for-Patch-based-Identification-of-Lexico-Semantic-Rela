...
from fetch_embeddings_image import get_vector_and_neighbors_embeddings, diffPatchAttention, productCos, productCos1
from fetch_embeddings_utils_image import get_words_neighbors_list, similarityMatrix1
...
def func_train_test_image(vectors, name_data, k, hps, file_paths):
    '''
    Params-
  ...
  #  try:
   ...
   #     print(err)
    ...
    if hps['concatNeigh']:
        if hps['diffPatchAtt']:
            print('diffPatchAtt')
            xtrainCoord1, ltrainCoord1, ltrainCoord2 = get_vector_and_neighbors_embeddings(wN1trainCoord, wN2trainCoord, k, hps)
            ...
            costrainCoord = diffPatchAttention(ltrainCoord1, ltrainCoord2, hps, k)
            ...
            xtestCoord1, ltestCoord1, ltestCoord2 = get_vector_and_neighbors_embeddings(wN1testCoord, wN2testCoord, k, hps)
            costestCoord = diffPatchAttention(ltestCoord1, ltestCoord2, hps, k)
            #if sequential architecture
            ...
            xtrainHyper1, ltrainHyper1, ltrainHyper2 = get_vector_and_neighbors_embeddings(wN1trainHyper, wN2trainHyper, k,  hps)
            costrainHyper = diffPatchAttention(ltrainHyper1, ltrainHyper2,hps, k)
            #if sequential architecture
            ...
            xtestHyper1, ltestHyper1, ltestHyper2 = get_vector_and_neighbors_embeddings(wN1testHyper, wN2testHyper, k, hps)
            costestHyper = diffPatchAttention(ltestHyper1, ltestHyper2,hps, k)
            #if sequential architecture
            ...
            xtrainRando1, ltrainRando1, ltrainRando2 = get_vector_and_neighbors_embeddings(wN1trainRando, wN2trainRando, k, hps)
            costrainRando = diffPatchAttention(ltrainRando1, ltrainRando2,hps, k)
            #if sequential architecture
            ...
            xtestRando1, ltestRando1, ltestRando2 = get_vector_and_neighbors_embeddings(wN1testRando, wN2testRando, k,  hps)
            costestRando = diffPatchAttention(ltestRando1, ltestRando2, hps, k)
            #if sequential architecture
            ...
            xtrainCoord = get_vector_and_neighbors_embeddings(wN1trainCoord, wN2trainCoord, k)
            xtestCoord= get_vector_and_neighbors_embeddings(wN1testCoord, wN2testCoord, k)

            xtrainHyper= get_vector_and_neighbors_embeddings(wN1trainHyper, wN2trainHyper, k)
            xtestHyper = get_vector_and_neighbors_embeddings(wN1testHyper, wN2testHyper, k)

            xtrainRando = get_vector_and_neighbors_embeddings(wN1trainRando, wN2trainRando, k)
            xtestRando = get_vector_and_neighbors_embeddings(wN1testRando, wN2testRando, k)
    ...
    if hps['diffPatch'] :
        ...
        for elem1, elem2 in zip(ltrainCoord1, ltrainCoord2):
            costrainCoord.append(similarityMatrix1(elem1+elem2)[:k+1, k+1:].reshape(1, -1)) # to avoid duplicate the data
        CtrainCoord = np.vstack((costrainCoord))
        print(CtrainCoord.shape) 

        costestCoord = []
        for elem1, elem2 in zip(ltestCoord1, ltestCoord2):
            costestCoord.append(similarityMatrix1(elem1+elem2)[:k+1, k+1:].reshape(1, -1))
        CtestCoord = np.vstack((costestCoord))
        print(CtestCoord.shape)

        costrainHyper = []
        for elem1, elem2 in zip(ltrainHyper1, ltrainHyper2):
            costrainHyper.append(similarityMatrix1(elem1+elem2)[:k+1, k+1:].reshape(1, -1))
        ...
        for elem1, elem2 in zip(ltestHyper1, ltestHyper2):
            s1 = similarityMatrix1(elem1+elem2)[:k+1, k+1:].reshape(1, -1)
        ...
        for elem1, elem2 in zip(ltrainRando1, ltrainRando2):
            costrainRando.append(similarityMatrix1(elem1+elem2)[:k+1, k+1:].reshape(1, -1))
        CtrainRando = np.vstack((costrainRando))
        print(CtrainRando.shape)

        costestRando = []
        for elem1, elem2 in zip(ltestRando1, ltestRando2):
            costestRando.append(similarityMatrix1(elem1+elem2)[:k+1, k+1:].reshape(1, -1))
