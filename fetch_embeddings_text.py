...
from fetch_embeddings_utils_text import get_words_neighbors_list, similarityMatrix, similarityMatrix1, cuboid_fn
...
def get_vector_and_neighbors_embeddings(embeddings_voca, wN1, wN2, k, glove_model, hps):
    ...
    for elem in list_words1:
        emb1 = []
        pp1 = similarityMatrix1(elem, glove_model)
        ...
        if hps['attNeigh']:  # This is INTRA patch attention.
            ...
            for e, r in zip(elem, pr):
                emb1.append(embeddings_voca[e] * pr[r])  # Re-weighted embeddings according to pagerank values
        else:
            for e in elem:
                emb1.append(embeddings_voca[e])  # Concatenating embeddings of neighbours with each other.
    ...
    for elem in list_words2:
        emb2 = []
        pp2 = similarityMatrix1(elem, glove_model)

        if hps['attNeigh']:
            g = nx.from_numpy_array(pp2)
            pr = nx.pagerank(g)
            for e, r in zip(elem, pr):
                emb2.append(embeddings_voca[e] * pr[r])
        else:
            for e in elem:
                emb2.append(embeddings_voca[e])
...
def diffPatchAttention(list_words1, list_words2, glove_model, hps, k):
    cosW = []
    for elem1, elem2 in zip(list_words1, list_words2):
        cosi = []
        s = similarityMatrix(elem1+elem2, glove_model)
...
def productCos1(embeddings_voca, list_words1, list_words2, pr):
    try :
        embw1N = []
        for elem in list_words1:
            emb1 = []
            for e in elem:
                emb1.append(embeddings_voca[e])
